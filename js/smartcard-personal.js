...
$(document).ready(function() {
    $('#twofactor_smartcard-submit-button').click(function() {
        ...
        if (password.length != 12) {
            $('#twofactor_smartcard-settings-msg').text("Message: Password needs to have exactly 12 characters!");
            $('#twofactor_smartcard-settings-msg').show();
            return;
        }

        showSpinner()
        var url = OC.generateUrl('/apps/twofactor_smartcard/settings/setPassword');
        ...
        $.when(request).done(function() {
            $('#twofactor_smartcard-settings-msg').text("Message: Password was saved.");
            $('#twofactor_smartcard-settings-msg').show();
        }).fail(function() {
            $('#twofactor_smartcard-settings-msg').text("Message: Some ERROR occured!");
            $('#twofactor_smartcard-settings-msg').show();
        });
    ...
    $('#twofactor_smartcard-delete-button').click(function() {
        $('#twofactor_smartcard-settings-msg').hide();
        showSpinner()
        var url = OC.generateUrl('/apps/twofactor_smartcard/settings/deletePassword');
        ...
        $.when(request).done(function() {
            $('#twofactor_smartcard-settings-msg').text("Message: Password was deleted.");
            $('#twofactor_smartcard-settings-msg').show();
        }).fail(function() {
            $('#twofactor_smartcard-settings-msg').text("Some ERROR occured!");
            $('#twofactor_smartcard-settings-msg').show();
        });
